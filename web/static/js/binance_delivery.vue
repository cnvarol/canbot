            ...
            <div class="card-body">
              <div class="row justify-content-md-center">
                <div class="col-md-4 border-right text-center">
                  ...
                  <h5>{{ asset.maintMargin|filter_price }} <small>{{ asset.asset }}</small></h5>
                </div>
                <div class="col-md-4 border-right text-center">
                  <h5 class="text-teal">Balance</h5>
                  <h5>{{ asset.marginBalance|filter_price }} <small>{{ asset.asset }}</small></h5>
                </div>
                <div class="col-md-4 text-center">
                  <h5 class="text-teal">Available</h5>
                  <h5>{{ asset.availableBalance|filter_price }} <small>{{ asset.asset }}</small></h5>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="card-body" id="memory">
              <div class="row justify-content-md-center">
                <div class="col-md-4 border-right text-center">
                  ...
                  <h5>{{ asset.walletBalance|filter_price }} <small>{{ asset.asset }}</small></h5>
                </div>
                <div class="col-md-4 border-right text-center">
                  <h5 class="text-teal">Unr. PNL</h5>
                  <h5>{{ asset.unrealizedProfit|filter_price }} <small>{{ asset.asset }}</small></h5>
                </div>
                <div class="col-md-4 text-center">
                  <h5 class="text-teal">Asset</h5>
                  <select class="form-control form-control-sm" style="padding:0; border:0; margin-top:-3px; font-size: 1.15rem; line-height: 1.2; text-align-last:center;" v-model="assetName" @change="onAssetChange($event)">
                    <option v-for="(item, index) in balances.info.assets" :key="index" :value="item.asset" :selected="item.asset === assetName">{{ item.asset }}</option>
                  </select>
                </div>
              </div>
            </div>
    ...
    <template>
      ...
      <vue-bootstrap4-table :rows="positions" :classes="dataTableClasses" :columns="positionsColumns" :config="dataTableConfig" :total-rows="positions.length">
        ...
        <template slot="symbol" slot-scope="props">
          ...
          <a target="blank" :href="'/tradingview/' + props.row.exchange + ':' + props.cell_value">{{ props.cell_value.replace('_PERP', '') }}</a> 
...
module.exports = {
  ...
  methods: {
    ...
    async onCancelAll() {
      ...
      const exchange = 'binance_delivery';
    ...
    async fetchPageAsJson() {
      const res = await fetch('/binance_delivery.json');
      if (res.status === 403) {
        window.location.href = '/login?return=/binance_delivery';
        return;
      }
      ...
      this.orders.forEach(o => {
        o.currency = parseFloat(o.order.amount);
